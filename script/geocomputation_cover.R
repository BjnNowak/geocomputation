...
#################
# Before making the map, we will extract "clean" boundaries from the UN
...
geom_sf(buff_list[[5]], mapping = aes(geometry = geom), fill = col_lv5,
  ...
  geom_sf(boundaries,mapping=aes(geometry=the_geom),color=alpha("white",0.05),lwd=0.15)+
  # Add capitals labels
