...
cp<-read_sf('https://github.com/BjnNowak/geocomputation/raw/main/data/ne_10m_populated_places.gpkg')%>%
  ...
  st_intersection(mp_with_countries%>%filter(continent=='Africa'))
...
# Color palette for Dorling cartogram
...
pal_ndvi <- scico(17, palette = 'acton')
pal_ndvi <- scico(17, palette = 'tokyo')
#pal_ndvi <- rev(scico(17, palette = 'imola'))

# Robinson bounding box
#xlims<-c(-2200000,4500000)
xlims<-c(-2200000,4800000) # Changing a bit the bounding box for the SpatRaster
